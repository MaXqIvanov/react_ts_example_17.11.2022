...
import { IUser, TAuthState } from '../../../ts/storeTypes';
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    changeCurrentCompany(state: TAuthState, action: PayloadAction<{elem: ICompany, setIsVisibleLabel: CallableFunction}>) {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getProfile.fulfilled, (state: TAuthState, { payload }: PayloadAction<{id_company: ICompany, response: {status: number, data: IUser}, response2: {data: ICompany[]}, params: {nav: NavigateFunction}}>) => {
      console.log(payload);
      state.loading = false;
    ...
    builder.addCase(userAuth.fulfilled, (state: TAuthState, { payload }: PayloadAction<{response: {status: number, data: {user: IUser, token: string, detail: string}}, params: {nav: any}}>) => {
      console.log(payload);
