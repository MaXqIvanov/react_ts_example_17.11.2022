...
export const SideBar = ({ isCollapseSideBar }: any) => {
  ...
  useEffect(() => {
    ...
    const navigation: any = sidebar_navigation_main.filter(
      (elem: any) => elem.links === current_path
    );
    if (navigation.length > 0) {
      setCurrentNavigationElem(navigation[0].id);
    }
    const navigation2: any = sidebar_navigation_company.filter(
      (elem: any) => elem.links === current_path
    );
    if (navigation2.length > 0) {
      setCurrentNavigationElem(navigation2[0].id);
    }
    const navigation3: any = sidebar_navigation_admin.filter(
      (elem: any) => elem.links === current_path
    );
  ...
  return (
    <div
      id="sidebar"
      className={`${styles.sidebar} ${isCollapseSideBar === true ? styles.sidebar_collapse : ''}`}
    >
      <div
        className={`${top !== 60 ? styles.sidebar_wrapper : styles.sidebar_wrapper_position} ${
          isCollapseSideBar === true ? styles.sidebar_collapse_wrapper : ''
        } `}
      >
        {sidebar_navigation_main.length > 0 &&
          sidebar_navigation_main.map(
            (elem: any) =>
              elem.access === true && (
                <div
                  onClick={() => {
                    setCurrentNavigationElem(elem.id);
                    router(`${elem.links}`);
                  }}
                  key={elem.id}
                  className={`${styles.navigation_main} ${
                    current_navigation_elem === elem.id && styles.navigation_main_active
                  }`}
                >
                  <div
        ...
        {isCollapseSideBar === false &&
          (sidebar_navigation_admin[0].access === true ||
            sidebar_navigation_admin[1].access === true) && (
            <div className={styles.category_company}>КОМПАНИЯ</div>
          )}
        {sidebar_navigation_company.length > 0 &&
          sidebar_navigation_company.map(
            (elem: any) =>
              elem.access === true && (
                <div
                  onClick={() => {
        ...
        {sidebar_navigation_admin.length > 0 &&
          sidebar_navigation_admin.map(
            (elem: any) =>
              elem.access === true && (
                <div
                  onClick={() => {
