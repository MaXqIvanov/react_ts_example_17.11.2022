...
import { changeEmployesCompanyAdmin, createEmployesCompanyAdmin, getEmployesCompanyAdmin } from '../../store/employesSlice';
...
export const SideBar = ({setIsVisibleSideBar, isvisible_sidebaer}:any) => {
  ...
  const {position_all_admin, employes_admin_current} = useSelector((state: RootState)=> state.employes)
        ...
        <div className={styles.user_side_menu_wrapper}>
            ...
            <div className={styles.modal_window_body}>
              <div
                ...
                <span>{(name?.split(' ')[0]?.split('')[0] ? name?.split(' ')[0]?.split('')[0]?.toUpperCase() : '') + (name?.split(' ')[1]?.split('')[0] ? name?.split(' ')[1]?.split('')[0]?.toUpperCase() : '')}</span>
              </div>
              <div style={{marginTop: '10px'}} className={'wrapper_input_percent'}>
                <input placeholder='ФИО' value={name} onChange={(e:any)=> setNamePosition(e.target.value)}/>
              </div>
              <InputMask onClick={()=> setIsVisibleLabel(false)} onBlur={()=> setIsVisibleLabel(false)} mask="+7 (999) 999-99-99" value={phoneHolder} onChange={((e:any) => {
                ...
                })} required placeholder={'Номер телефона'} className={`${styles.input_phone}`} />
              <div style={{marginTop: '10px'}} onClick={()=> setIsVisibleSelect(!isVisibleSelect)} className={'custom_select_wrapper_admin'}>
                ...
                {isVisibleSelect &&
                  <div ref={select} className={'select_additional'}>
                    ...
                    position_all_admin.map((elem:any)=> <div onClick={()=> setPosition(elem)} className={'position_current'}>
                      <span>{elem.name}</span>
                    </div>)}
                  </div>
                }
            ...
            <div style={{display: 'flex' , justifyContent: 'right', width: '100%', marginRight: '30px'}} className={'custom_btn_wrapper'}>
                <div onClick={()=> setIsVisibleSideBar(!isvisible_sidebaer)} className={'btn_cancel'}><span>Отмена</span></div>
                <div onClick={()=> dispatch(changeEmployesCompanyAdmin({
                  phone: phoneHolder,
                  name: name,
